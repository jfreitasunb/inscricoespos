...
class RelatorioController extends BaseController
{
  ...
  public function ConsolidaCabecalhoCSV()
  {
    ...
    return $cabecalho = ["Nome", "Tipo de Concorrência", "E-mail", "Programa Pretendido", "Vínculo Empregatício", "Interesse em bolsa", "Local da graduação ", "Local do Mestrado ", "Área Principal", "Área Secundária"];
  }
  ...
  public function geraRelatorio($id_inscricao_pos)
  {
    ...
    foreach ($usuarios_finalizados as $candidato) {
      ...
      $linha_arquivo['vinculo_empregaticio'] = $dados_candidato_para_relatorio['vinculo_empregaticio']? "Sim" : "Não";

      $linha_arquivo['interesse_bolsa'] = $dados_candidato_para_relatorio['interesse_bolsa']? "Sim" : "Não";

      $linha_arquivo['instituicao_graduacao'] = $dados_candidato_para_relatorio['instituicao_graduacao'];

      if ($dados_candidato_para_relatorio['programa_pretendido'] == "Doutorado") {
        $linha_arquivo['instituicao_pos'] = $dados_candidato_para_relatorio['instituicao_pos'];
        
        $linha_arquivo['area_pos_principal'] = $dados_candidato_para_relatorio['area_pos_principal'];

        $linha_arquivo['area_pos_secundaria'] = $dados_candidato_para_relatorio['area_pos_secundaria'];
      }else{
        $linha_arquivo['instituicao_pos'] = "Não se aplica";
        
        $linha_arquivo['area_pos_principal'] = "Não se aplica";

        $linha_arquivo['area_pos_secundaria'] = "Não se aplica";
      }
