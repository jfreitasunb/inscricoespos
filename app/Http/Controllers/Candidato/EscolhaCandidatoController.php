	...
	public function getEscolhaCandidato()
	{
		...
		if ($autoriza_inscricao) {
			...
			if (in_array(2, $programas_disponiveis)) {
				...
				$areas_pos = AreaPosMat::where('id_area_pos', '!=', 3)->where('id_area_pos', '!=', 4)->where('id_area_pos', '!=', 10)->pluck($nome_coluna,'id_area_pos')->prepend(trans('mensagens_gerais.selecionar'),'');
